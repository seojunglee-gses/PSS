...
export default function Workspace() {
  ...
  const hasAutoGeneratedAlternatives = useRef(false);
  ...
  useEffect(() => {
    ...
    if (hasAutoGeneratedAlternativePrompt.current) {
      return;
    }
  ...
  const requestAutoGeneratedImage = useCallback(async () => {
    ...
    if (hasAutoGeneratedAlternatives.current) {
      return;
    }
    if (!hasLoadedChatLogs) {
      return;
    ...
    hasAutoGeneratedAlternatives.current = true;
    requestAutoGeneratedImage().catch((error) => {
      setErrorMessage(
